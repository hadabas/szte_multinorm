...
class SparseAutoAttack(object):
    ...
    def untarget_attack(self, att, loader, clean=False):
        x_next, y_next, idx_next = None, None, None
        if clean:
            clean_acc = 0.
        ...
        for x, y, idx in tqdm(loader):
            ...
            if ind_fail.numel() > 0:
                ...
                try:
                    ...
                    idx_fail = idx_fail[ind_untargeted_fail.cpu()]
                except:
            ...
            if ind_fail.numel() > 0:
                ...
                idx_next = idx_fail if idx_next is None else torch.cat((idx_next, idx_fail), dim=0)
            if clean:
                clean_acc += c_acc
            untarget_robust_acc += unt_r_acc
        if clean:
            return x_next, y_next, idx_next, clean_acc, untarget_robust_acc
        else:
            return x_next, y_next, idx_next, untarget_robust_acc
    ...
    def black_attack(self, att, loader):
        ...
        with torch.no_grad():
            for x, y, idx in tqdm(loader):
                ...
                pred = output.max(1)[1]
                ind_robust = (pred == y).nonzero(as_tuple=True)[0]
                if ind_robust.numel() > 0:
                    robust_idx.append(idx[ind_robust.cpu()])
        ...
        if len(robust_idx) > 0:
            robust_idx = torch.cat(robust_idx, dim=0)
        else:
            robust_idx = torch.empty(0, dtype=torch.long)

        robust_acc = robust_acc_for_black
        return robust_acc, robust_idx

    def __call__(self, loader):
        N = len(loader.dataset)
        robust_mask = torch.zeros(N, dtype=torch.bool)
        ###################################
        #### Untargeted Attack 1 ##########
        ###################################
        time_used = 0.
        ...
        st = time.time()
        x_for_unt_2, y_for_unt_2, idx_for_unt_2, clean_acc, untarget_robust_acc_1 = self.untarget_attack(self.att1, loader, clean=True)
        ed = time.time()
        time_used += ed - st
        ...
        if x_for_unt_2 is None:
            return clean_acc, 0., robust_mask, time_used
        ...
        dataset_for_unt_2 = TensorDataset(x_for_unt_2, y_for_unt_2, idx_for_unt_2)
        ...
        #                                                                        clean=False)
        x_for_black, y_for_black, idx_for_black, untarget_robust_acc_2 = self.untarget_attack(self.att2, loader=loader_for_unt_2,
                                                                               clean=False)
        ed = time.time()
        time_used += ed - st
        ...
        dataset_for_black = TensorDataset(x_for_black, y_for_black, idx_for_black)
        print('# White-box 2 targeted attack failed samples:', len(dataset_for_black))
        loader_for_black = DataLoader(dataset_for_black,
                                      batch_size=2*loader.batch_size,
                                      shuffle=False,
                                      num_workers=2)
        st = time.time()
        black_robust_acc, globalidx_black_robust = self.black_attack(self.black, loader=loader_for_black)
        ed = time.time()
        time_used += ed - st

        robust_mask[globalidx_black_robust] = True

        print('Final L0 robust accuracy (with 3 attacks): {:.2f}%'.format(black_robust_acc / len(loader.dataset) * 100))
        return clean_acc, black_robust_acc, robust_mask, time_used
