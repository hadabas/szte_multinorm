    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == 'L0':
                ...
                for it in range(1, self.n_queries):
                    ...
                    ind_fail = (margin_min > 0).nonzero().squeeze()
                    if self.verbose and ind_succ.numel() != 0:
                        if (it+1) % self.verbose_interval == 0:
                            self.acc_list.append(ind_fail.numel())
                            self.ind_fail_list.append(ind_fail.cpu().numpy())
        ...
        if self.verbose:
            if len(self.acc_list) == 0:
                self.acc_list = [0] * (self.n_queries // self.verbose_interval)
                self.ind_fail_list = [np.array([])] * (self.n_queries // self.verbose_interval)
            if len(self.acc_list) != self.n_queries // self.verbose_interval:
                self.acc_list += [self.acc_list[-1]] * (self.n_queries // self.verbose_interval - len(self.acc_list))
                self.ind_fail_list += [self.ind_fail_list[-1]] * (self.n_queries // self.verbose_interval - len(self.ind_fail_list))
            return n_queries, x_best, self.acc_list, self.ind_fail_list
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                if self.verbose:
                    qr_curr, adv_curr, acc_list, ind_fail_list = self.attack_single_run(x_to_fool, y_to_fool)
                    ind_fail_list = [ind_to_fool[i].cpu().numpy() for i in ind_fail_list]
                else:
                ...
                #         time.time() - startt))
        if self.verbose:
            return qr, adv, acc_list, ind_fail_list
        return qr, adv
