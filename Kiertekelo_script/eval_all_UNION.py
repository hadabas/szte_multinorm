...
from union_models.preact_resnet import PreActResNet18 as union_load_preact_resnet
...
if __name__ == "__main__":
    ...
    # ---- load model ---- !!ÁLTALÁBAN EZT KELL ÁLLÍTANI!!! (Unionnál kézzel át kell írni a modell definíciós fájlba a num_classes-t, ha más adatbázisra akarjuk tesztelni.)
    if args.dataset == "cifar10":
        model = union_load_preact_resnet().cuda()
    elif args.dataset == "cifar100":
        model = union_load_preact_resnet().cuda()
    else:
        model = union_load_preact_resnet().cuda()
