...
eps_dict = {'cifar100': {'Linf': 8. / 255., 'L2': .5, 'L1': 12.},
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='../../Adatbazis_mappak/cifar100', help='where to store downloaded datasets')
...
def main():
    ...
    if args.dataset == 'cifar100':
        train_loader, _ = data.load_cifar100_train(args, only_train=True)
        
        # non augmented images for statistics
        x_train_eval, y_train_eval = data.load_cifar100(args.n_ex_eval,
            args.data_dir, training_set=True, device='cuda')
        x_test_eval, y_test_eval = data.load_cifar100(args.n_ex_eval,
    ...
    if not args.finetune_model:
        ...
        if args.wide:
            ...
            model = PreActResNet18(100, activation=args.act).cuda()
        model.eval()
        ...
        model = PreActResNet18(100, activation=args.act)
    ...
    if args.final_eval:
        if args.dataset == 'cifar100':
            x, y = data.load_cifar100(args.n_ex_final, data_dir=args.data_dir, device='cpu')
