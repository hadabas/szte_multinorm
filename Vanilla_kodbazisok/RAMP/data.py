...
def load_cifar100(n_examples, data_dir='/shared/jhadabas/Baseline_meresek/Adatbazis_mappak/Cifar100', training_set=False, device='cuda'):
    transform_chain = transforms.Compose([transforms.ToTensor()])
...
def load_cifar100_train(args, only_train=False):
    train_transform = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
    ...
    # subset használata ha n_examples meg van adva
    if args.n_examples != 0:
        subset_indices = list(range(args.n_examples))
