...
# python3 test.py -gpu_id 0 -model 3 -batch_size 1 -attack 4 -restarts 10
...
parser.add_argument("-attack", help = "Foolbox = 0; Custom PGD = 1, Min PGD = 2, Fast DDN = 3, AutoAttack = 4", type = int, default = 0)
...
def run_autoattack(model_name):
    print("Running AutoAttack on", model_name)
    torch.manual_seed(0)
    model = PreActResNet18().cuda()
    for m in model.children(): 
        if not isinstance(m, nn.BatchNorm2d):
    ...
    for batch in test_batches:
        x_test.append(batch['input'].float().to(device))
        y_test.append(batch['target'].long().to(device))
...
model_list = ["LINF", "L1", "L2", "MSD", "TRIPLE", "WORST", "VANILLA"]
