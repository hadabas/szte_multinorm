...
def pgd_l1_topk(model, X,y, epsilon = 12, alpha = 0.05, num_iter = 50, k = 20, device = "cuda:0", restarts = 1, version = 0):
...
# RANDOM implementáció (Minden kérdéses batch-nél randomol támadástípust L1, L2 és Linf között. Ezt a módszert eléggé befolyásolja a batch mérete.)
def random_adv(loader, lr_schedule, model, epoch_i, attack,
               criterion=nn.CrossEntropyLoss(),
               opt=None,
               device="cuda:0",
               epsilon_l_1=12,
    ...
    for i, batch in enumerate(loader):
        ...
        attack_type = torch.randint(0,3,(1,)).item()
