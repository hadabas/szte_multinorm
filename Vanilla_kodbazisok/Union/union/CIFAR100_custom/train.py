...
# GPT szerint MAX == worst(5); AVG == triple(4); RND == random(7); MSD == msd_v0(3).
parser = argparse.ArgumentParser(description='Custom Adversarial Training for CIFAR100', formatter_class=argparse.RawTextHelpFormatter)
parser.add_argument("-gpu_id", help="Id of GPU to be used", type=int, default = 0)
parser.add_argument("-model", help="Type of Adversarial Training: \n\t 0: l_inf \n\t 1: l_1 \n\t 2: l_2 \n\t 3: msd \n\t 4: triple \n\t 5: worst \n\t 6: vanilla \n\t 7: random", type=int, default = 3)
...
train_batches = Batches(train_set_x, batch_size, shuffle=True, set_random_choices=True, num_workers=4, gpu_id = torch.cuda.current_device())
test_batches = Batches(test_set, batch_size, shuffle=False, num_workers=4, gpu_id = torch.cuda.current_device())
...
attack_list = [ pgd_linf ,  pgd_l1_topk,   pgd_l2 ,  msd_v0 ,  triple_adv ,  pgd_worst_dir, triple_adv, triple_adv] #TRIPLE, VANILLA DON'T HAVE A ATTACK NAME ANYTHING WORKS
attack_name = ["pgd_linf", "pgd_l1_topk", "pgd_l2", "msd_v0", "triple_adv", "pgd_worst_dir", "vanilla", "Random(pgd_linf|pgd_l1_topk|pgd_l2)"]
folder_name = ["LINF", "L1", "L2", "MSD_2", "TRIPLE_2", "WORST_0", "VANILLA", "RANDOM_2"]
...
for epoch_i in range(1,epochs+1):  
    ...
    if choice == 6:
        ...
        train_loss, train_acc = random_adv(train_batches, lr_schedule, model, epoch_i, attack, criterion, opt = opt, device = device, epsilon_l_2 = 0.5, epsilon_l_inf = 0.03137)
    else:
